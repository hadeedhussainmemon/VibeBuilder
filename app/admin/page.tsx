              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/10 text-xs font-black uppercase tracking-widest text-gray-500 bg-white/[0.03]">
                    ...
                    <th className="px-8 py-6">Usage (Builds)</th>
                    <th className="px-8 py-6">Max Limit</th>
                    <th className="px-8 py-6 text-right">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {data?.users.map((user: any) => (
                    <tr key={user._id} className="group hover:bg-white/[0.03] transition-colors">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          <img src={user.image} className="w-8 h-8 rounded-full border border-white/10" />
                          <div>
                            <p className="text-sm font-bold">{user.name}</p>
                            <p className="text-[10px] text-gray-500 font-mono tracking-tight">{user.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2">
                           <div className="w-24 h-1.5 bg-white/5 rounded-full overflow-hidden">
                              <div 
                                ...
                                style={{ width: `${Math.min((user.sitesCount / user.maxSites) * 100, 100)}%` }} 
                              />
                           </div>
                           <span className="text-xs font-bold text-gray-400">{user.sitesCount}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <span className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest">
                          {user.maxSites || 5} Total
                        </span>
                      </td>
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <input 
                            type="number" 
                            defaultValue={user.maxSites || 5}
                            id={`limit-${user._id}`}
                            className="w-16 bg-white/5 border border-white/10 rounded-lg py-1 px-2 text-xs text-center outline-none focus:border-purple-500/50"
                          />
                          <button 
                            onClick={async () => {
                              const newLimit = (document.getElementById(`limit-${user._id}`) as HTMLInputElement).value;
                              const res = await fetch("/api/admin/users/update-credits", {
                                method: "POST",
                                body: JSON.stringify({ userId: user._id, maxSites: parseInt(newLimit) })
                              });
                              if (res.ok) {
                                window.location.reload();
                              }
                            }}
                            className="px-4 py-1.5 rounded-lg bg-white text-black text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-transform"
                          >
                            Set Limit
                          </button>
                        </div>
